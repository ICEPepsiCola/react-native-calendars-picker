...
export const areDatesSameOrAfter = (a: DateType, b: DateType) => {
  return dayjs(a).isSameOrAfter(dayjs(b));
};

export const areDateBetweenOrAt = (a: DateType, b: DateType[]) => {
  if (!a || !b) {
    return false;
  }
  if (b.length !== 2) {
  ...
  if (!b[0] || !b[1]) {
    return areDatesOnSameDay(a, b[0] || b[1]);
  }
  return dayjs(a).isBetween(dayjs(b[0]), dayjs(b[1]), 'day', '[]');
};

export const unshiftHistory = (history: DateType[], date: DateType) => {
  const arr = [];
  for (let i = 0; i < history.length; i++) {
    if (areDatesOnSameDay(history[i], date)) {
      continue;
    }
    arr.push(history[i]);
  }
